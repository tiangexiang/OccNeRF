...
def rays_intersect_3d_bbox(bounds, ray_o, ray_d, use_mask=True):
    ...
    if not use_mask:
        mask_at_box = np.ones_like(mask_at_box).astype(np.bool)
    p_intervals = p_intersect[mask_at_box][p_mask_at_box[mask_at_box]].reshape(
        -1, 2, 3) # (N_VALID_rays, 2, 3)
    #print(p_intervals.shape)
...
def rays_intersect_mesh(verts, ray_o, ray_d):
    r"""calculate intersections with 3d bounding box
        Args:
            - verts: (6890, 3)
            - ray_o: (N_rays, 3)
