...
class Dataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        print(type(self.train_camera), self.train_camera.keys(), '???')
    ...
    def load_train_cameras(self):
        cameras = None
        with open(os.path.join(self.dataset_path, 'all_cameras.pkl'), 'rb') as f: 
    ...
    def load_train_mesh_infos(self):
        mesh_infos = None
        if os.path.exists(os.path.join(self.dataset_path, 'all_mesh_infos.pkl')):
            self.has_all_mesh = True
            with open(os.path.join(self.dataset_path, 'all_mesh_infos.pkl'), 'rb') as f:   
                mesh_infos = pickle.load(f)

            for frame_name in mesh_infos.keys():
                all_bbox = []
                for cidx in range(mesh_infos[frame_name]['joints'].shape[0]):
                    bbox = self.skeleton_to_bbox(mesh_infos[frame_name]['joints'][cidx])
                    all_bbox.append(bbox)
                mesh_infos[frame_name]['bbox'] = np.stack(all_bbox, axis=0)
        else:
    ...
    def load_image(self, frame_name, bg_color, idx):
        ...
        if 'distortions' in self.train_camera:
            K = self.train_camera['intrinsics'][idx]
            D = self.train_camera['distortions'][idx]
    ...
    def __getitem__(self, idx):
        ...
        img, _ = self.load_image(frame_name, bgcolor, idx)
        img = img / 255.
        H, W = img.shape[0:2]

        dst_skel_info = self.query_dst_skeleton()
        if self.has_all_mesh:
            dst_bbox = dst_skel_info['bbox'][idx]
            dst_poses = dst_skel_info['poses'][idx]
            dst_tpose_joints = dst_skel_info['dst_tpose_joints'][idx]
            dst_Rh = dst_skel_info['Rh'][idx]
            dst_Th = dst_skel_info['Th'][idx]
        else:
        ...
        #                 trans=dst_Th)
        K = self.train_camera['intrinsics'][idx, :3, :3].copy()
        K[:2] *= cfg.resize_img_scale

        E = self.train_camera['extrinsics'][idx]
        ...
        near, far, ray_mask = rays_intersect_3d_bbox(dst_bbox, rays_o, rays_d, use_mask=True)
        #ray_mask = np.ones_like(ray_mask)
        ...
        if 'rays' in self.keyfilter:
            results.update({
                ...
                'far': far,
                'bgcolor': bgcolor})
        #print(near.shape, far.shape, rays_o.shape, rays_d.shape)
