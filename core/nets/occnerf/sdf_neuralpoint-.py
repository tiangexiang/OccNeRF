...
class NeuralPoint(nn.Module):
    def __init__(self, verts, pos_embed_fn=None, num_levels=5, level_distance=0.02, learn_points=True, 
        ...
        self.point_cloud.append(nn.Parameter(torch.empty_like(verts).float(), requires_grad=learn_points))
        ...
        for i in range(1, num_levels):
            ...
            self.point_cloud.append(nn.Parameter(torch.empty_like(low_res_verts).float(), requires_grad=learn_points))
            #self.point_cloud[-1].data = low_res_verts.clone()
    ...
    def query(self, xyz, k, return_index=False, point_cloud=None, return_normals=False):
