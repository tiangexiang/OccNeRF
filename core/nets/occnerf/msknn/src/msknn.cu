...
template <typename scalar_t>
__global__ void
msknn_kernel(const scalar_t *__restrict__ x, const scalar_t *__restrict__ y,
           const int64_t *__restrict__ ptr_x_index, 
           const int64_t *__restrict__ ptr_index, 
           const int64_t *__restrict__ ptr_x, const int64_t *__restrict__ ptr_y,
           int64_t *__restrict__ row, int64_t *__restrict__ col,
           const int64_t k, const int64_t n, const int64_t m,
           const int64_t num_examples, const bool cosine) {
...
//   tmp_ptr_x = ptr_x[]
  ...
  for (int64_t n_x = ptr_x[example_idx]; n_x < ptr_x[example_idx + 1]; n_x++) {
    if (ptr_x_index[n_x] != ptr_index[n_y]){
        continue;
    }
...
torch::Tensor msknn(const torch::Tensor x, const torch::Tensor y,
  ...
  CHECK_INPUT(index.dim() == 2);

  CHECK_CUDA(x_index);
  CHECK_CONTIGUOUS(x_index);
  CHECK_INPUT(x_index.dim() == 1);
  ...
  ptr_index = torch::arange(0, index.size(0) + 1, index.size(0),
                          index.options().dtype(torch::kLong));

  CHECK_INPUT(ptr_x.value().numel() == ptr_y.value().numel());

  cudaSetDevice(x.get_device());

  auto index_row = torch::empty({y.size(0) * 1}, ptr_y.value().options());
  auto index_col = torch::full(y.size(0) * 1, -1, ptr_y.value().options());
  ...
  auto scalar_type = index.scalar_type();
  AT_DISPATCH_FLOATING_TYPES_AND(at::ScalarType::Half, scalar_type, "_", [&] {
    knn_kernel<scalar_t><<<BLOCKS, THREADS, 0, stream>>>(
        index.data_ptr<scalar_t>(), y.data_ptr<scalar_t>()
        ptr_index.value().data_ptr<int64_t>(), ptr_y.value().data_ptr<int64_t>(),
        index_row.data_ptr<int64_t>(), index_col.data_ptr<int64_t>(), 1, index.size(0),
        y.size(0), index.size(1), ptr_index.value().numel() - 1, cosine);
  });
  auto mask = index_col != -1;
  index_col = index_col.masked_select(mask); //N, 1

  // orignal
  auto stream = at::cuda::getCurrentCUDAStream();
  auto scalar_type = x.scalar_type();
  AT_DISPATCH_FLOATING_TYPES_AND(at::ScalarType::Half, scalar_type, "_", [&] {
    msknn_kernel<scalar_t><<<BLOCKS, THREADS, 0, stream>>>(
        x.data_ptr<scalar_t>(), y.data_ptr<scalar_t>(),
        x_index.data_ptr<int64_t>(),
        index_col.data_ptr<int64_t>(),
        ptr_x.value().data_ptr<int64_t>(), ptr_y.value().data_ptr<int64_t>(),
        row.data_ptr<int64_t>(), col.data_ptr<int64_t>(), k, x.size(0),
        y.size(0), index.size(0), x.size(1), ptr_x.value().numel() - 1, cosine);
  });
